 ...
 */
include_once('Mage/Sales/Model/Quote/Address.php');
class Concalma_Dropship_Model_Quote_Address extends Mage_Sales_Model_Quote_Address
{
 ...
 //del dev
    ...
    public function getAllWarehouses () {
    	$itemscollection = $this->getAllItems();
     ...
     */
        public function requestShippingRatesForDropship(Mage_Sales_Model_Quote_Item_Abstract $item = null, $w= null)
    {
        /** @var $request Mage_Shipping_Model_Rate_Request */
        $request = Mage::getModel('shipping/rate_request');
        $request->setAllItems($item ? array($item) : $this->getAllItems());
 ...
 //del dev
    ...
    /***
     * how to pass the configuration data such as the ups account
     */
    
    public function getDropshipRateResult(Mage_Sales_Model_Quote_Item_Abstract $item = null, $weight, $warehouseTitle) {
    	$request = Mage::getModel('shipping/rate_request');
        $request->setAllItems($item ? array($item) : $this->getAllItems());
        $request->setDestCountryId($this->getCountryId());
        $request->setDestRegionId($this->getRegionId());
      ...
      //  if ($w != null) {
       ...
       // }
    	...
    	$result = Mage::getModel('shipping/shipping')->collectCarrierRatesDropship($warehouseTitle,$request, $weight );
    	return $result;
    }
