 ...
 * obtain it through the world-wide-web, please send an email  nurture business work log h for l p in hazardous condition
 ...
 */
include_once('Mage/Usa/Model/Shipping/Carrier/Dhl.php');
class Concalma_Dropship_Model_Shipping_Carrier_Dhl extends Mage_Usa_Model_Shipping_Carrier_Dhl
    ...
    implements Mage_Shipping_Model_Carrier_Interface
{
    ...
    {
    	//hook by luuthuy205@gmail.com hungnamimportant must alter table dropship to add the dhl account, password, ...
        if ($request->getData('warehouse') != '' && $request->getData('warehouse') != null) {
        	$warehouseInfo = Mage::helper('dropship')->getWarehouseInformation($request->getData('warehouse'));
        	
        	//set the data
        	 $request->setDhlId($warehouseInfo['dhl_id']);
        	 $request->setDhlPassword($warehouseInfo['dhl_password']);
        	 $request->setDhlAccount($warehouseInfo['dhl_account']); //getDhlAccount
        	 $request->setDhlShippingKey($warehouseInfo['dhl_shipping_key']); //getDhlAccount
        	 $request->setDhlShippingIntlKey($warehouseInfo['dhl_shipping_intl_key']); //getDhlAccount
        	 $request->setDhlShipmentType($warehouseInfo['dhl_shipping_type']); //getDhlAccount
        	 $request->setDhlDutiable($warehouseInfo['dhl_dutibale']); //getDhlAccount  
        	// getDhlDutiable
        }
    	//end of hook
        ...
        if ($request->getDhlShippingKey()) {
            ...
            $shippingKey = $this->getConfigData('shipping_key'); //already p
        }
        $r->setShippingKey($shippingKey);

        if ($request->getDhlShippingIntlKey()) {
            $shippingKey = $request->getDhlShippingIntlKey(); //alreay p
        } else {
